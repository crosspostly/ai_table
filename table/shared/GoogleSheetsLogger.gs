...
function getUserEmail_() {
  try {
    ...
    // Если не хватает прав, логируем и пытаемся исправить
    if (e.message && e.message.includes('insufficient permissions')) {
      addSystemLog('⚠️ Требуется авторизация для получения email пользователя.', 'WARN', 'AUTH');
      
      // Пытаемся автоматически проверить и запустить авторизацию
      try {
        if (typeof autoCheckAndAuthorize === 'function') {
          autoCheckAndAuthorize();
        } else {
          // Fallback: пытаемся инициировать авторизацию напрямую
          ScriptApp.getAuthorizationInfo(ScriptApp.AuthMode.FULL);
        }
      } catch (authError) {
        // Игнорируем ошибку, так как нас интересует только показ диалога
        addSystemLog('Не удалось автоматически запустить авторизацию. Используйте меню: ⚙️ Настройки → 🚀 Запустить авторизацию', 'WARN', 'AUTH');
      }
    }
    return 'unknown';
  }
