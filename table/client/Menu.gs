...
function onOpen() {
  ...
  // Главное меню с основными рабочими функциями
  ui.createMenu('🤖 Table AI')
    ...
    .addSeparator()
    .addSubMenu(ui.createMenu('📱 VK и Социальные сети')
      .addItem('📱 Импорт постов VK', 'importVkPosts')
      .addItem('🔗 Импорт из Instagram', 'importInstagramPosts')
      .addItem('💬 Импорт из Telegram', 'importTelegramPosts')
      .addItem('📊 Настройки соцсетей', 'configureSocialImport'))
    .addSubMenu(ui.createMenu('🔗 Умные цепочки')
      .addItem('🚀 Запустить цепочку A3→B3→C3...', 'runSmartChain')
      .addItem('⚡ Быстрая цепочка (текущая строка)', 'runChainCurrentRow')
      .addItem('🔧 Настроить цепочку', 'configureSmartChain')
      .addItem('📋 Статус цепочки', 'showChainStatus'))
    .addSubMenu(ui.createMenu('📝 Отзывы и OCR')
      .addItem('📷 Распознать изображения (OCR)', 'runOcrProcessing')
      .addItem('📝 Обработать отзывы', 'processReviews')
      .addItem('🔄 Автообработка новых отзывов', 'enableAutoReviewProcessing')
      .addItem('⚙️ Настройки OCR', 'configureOcrSettings'))
    .addSeparator()
