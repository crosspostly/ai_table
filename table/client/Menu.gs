...
function onOpen() {
  ...
  ui.createMenu('🤖 Table AI')
    ...
    .addItem('🔄 Обновить данные в ячейке', 'runChainCurrentRow') 
    .addItem('📱 Получить VK посты', 'importVkPosts')
    .addItem('💬 Анализ отзывов', 'ocrRun')
    .addSeparator()
    .addItem('🧠 Режим чата', 'initializeChatMode')
    .addItem('⚡ Активировать умные промпты', 'setupSmartPromptTrigger')
    .addSeparator()
    .addSubMenu(ui.createMenu('⚙️ Настройки')
      ...
      .addItem('🧹 Очистить формулы B3..G3', 'clearChainForA3')
      .addSeparator()
      ...
      .addSeparator()
      .addItem('📊 Статус системы', 'checkSystemStatus'))
