...
/**
 * Получить VK посты - с инструкцией
 */
function importVkPostsWithHelp() {
  var ui = SpreadsheetApp.getUi();
  var instruction = `📱 ПОЛУЧИТЬ VK ПОСТЫ

Импортирует посты из группы VK с автоматической фильтрацией:

📋 Что делает:
• Загружает посты из указанной VK группы
• Создает формулы для фильтрации по стоп-словам
• Добавляет фильтр по позитивным словам

📝 Подготовка:
1. Создайте лист "Параметры" (если нет)
2. В ячейку B1 введите ID группы (например: -12345678)
3. В ячейку B2 введите количество постов (до 100)

📝 Использование:
• В колонку E добавьте стоп-слова (посты с ними скроются)
• В колонку H добавьте позитивные слова (посты с ними выделятся)

⚡ Результат создается в листе "посты"`;
