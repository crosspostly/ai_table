...
function onOpen() {
  ...
  // ТИХАЯ ПРОВЕРКА АВТОРИЗАЦИИ при запуске (только логирование)
  try {
    if (typeof checkAuthorizationStatus === 'function') {
      var authStatus = checkAuthorizationStatus();
      if (!authStatus.authorized && authStatus.errors.length > 0) {
        // Логируем проблему без показа алерта
        if (typeof addSystemLog === 'function') {
          addSystemLog(
            '⚠️ Обнаружены проблемы с авторизацией: ' + authStatus.errors.join(', ') + 
            '. Используйте меню: ⚙️ Настройки → 🔐 Управление авторизацией',
            'WARN',
            'AUTH'
          );
        }
    ...
    // Игнорируем ошибки проверки авторизации при старте
    console.log('Автоматическая проверка авторизации пропущена: ' + e.message);
  }
  ...
  ui.createMenu('🤖 Table AI')
    ...
    .addSubMenu(ui.createMenu('⚙️ Настройки')
      ...
      .addSeparator()
      .addItem('🔐 Управление авторизацией', 'openAuthorizationDialog')
      .addItem('🚀 Запустить авторизацию', 'initiateAuthorizationFlow')
      .addItem('🧪 Тест авторизации', 'testAuthorization')
      .addSeparator()
