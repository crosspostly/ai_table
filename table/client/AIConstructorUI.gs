/**
 * AI Constructor UI Functions
 * Функции для работы с веб-интерфейсом настройки AI запроса
 * 
 * Version: 1.1.0 - RENAMED FROM CollectConfigUI для диагностики проблем синхронизации
 ...
 */
function openAIConstructorUI() {
  try {
    var html = HtmlService.createHtmlOutputFromFile('AIConstructorUI')
      .setWidth(650)
      .setHeight(600)
 ...
 */
function getAIConstructorInitData() {
  try {
 ...
 */
function refreshCellWithAIConstructor() {
  try {
    ...
    var config = loadAIConstructorConfig(sheetName, cellAddress);
    ...
    // Выполняем запрос
    var result = executeAIConstructorConfig(sheetName, cellAddress);
 ...
 * @return {boolean}
 */
function hasAIConstructorForCurrentCell() {
  try {
    var sheet = SpreadsheetApp.getActiveSheet();
    var range = sheet.getActiveRange();
 ...
 */
function showAIConstructorHelp() {
  var ui = SpreadsheetApp.getUi();
 ...
 */
function testAIConstructorUI() {
  try {
    var ui = SpreadsheetApp.getUi();
    ui.alert('✅ AI Constructor UI тест', 'Функция AIConstructorUI работает правильно!', ui.ButtonSet.OK);
    return true;
  } catch (error) {
    Logger.log('ERROR in testAIConstructorUI: ' + error.message);
    return false;
  }
}